...
- 자동보정 버튼으로 기존 툴 값을 분석 기반 추천값으로 설정
- 별 마스크 감지 및 은하/성운 보호형 별 축소/별 제거
- 은하/성운 후보 자동 감지
- 사용자가 이미지 위를 드래그해 대상 영역 힌트 지정
- 천체/배경/별 3종 마스크 기반 부분 보정
- 별 제거 기능을 별도 Star Tools 메뉴로 분리하고, 별 제거본/별 레이어 확인 지원
- 자동보정은 별 제거를 적용하지 않고 약한 별 축소까지만 추천
- View 기본 모드에서는 사진만 표시하고, 상단 툴바에서 필요한 도구 패널만 열기
